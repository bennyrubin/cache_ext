...
#include "cache_ext_agent.skel.h"

char *USAGE = "Usage: ./cache_ext_agent --watch_dir <dir> --cgroup_path <path>\n";
struct cmdline_args {
	char *watch_dir;
	char *cgroup_path;
};
...
int main(int argc, char **argv) {
	...
	struct cache_ext_agent_bpf *skel = NULL;
	...
	skel = cache_ext_agent_bpf__open();
	if (!skel) {
	...
	if (cache_ext_agent_bpf__load(skel)) {
	...
	link = bpf_map__attach_cache_ext_ops(skel->maps.agent_ops, cgroup_fd);
	if (link == NULL) {
	...
	if (cache_ext_agent_bpf__attach(skel)) {
...
cleanup:
	...
	cache_ext_agent_bpf__destroy(skel);
	return ret;
}
